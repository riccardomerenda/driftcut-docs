...
!!! note "Live call reliability"
    In `v0.8.0`, live `run` calls automatically retry transient rate limits, timeouts, connection failures, and 5xx responses before Driftcut records an `api_error`. Saved JSON artifacts include `retry_count` for each baseline/candidate response.
...
!!! note "Current alpha behavior"
    `min_batches` is active in `v0.8.0`: Driftcut will not declare `PROCEED` until at least this many batches have been evaluated.
...
!!! note "Current alpha behavior"
    Latency is measured and reported today. The thresholds are active decision inputs in `v0.8.0`.
